...
typedef struct
{
	const float shuntResistor[4] = {3.33, 3.35, 3.38, 3.4};			  // Array for each Shunt Resistors actual measured resistance
	const float chargeLedPinMidVolatge[4] = {2.04, 1.99, 2.08, 2.02}; // Array for each Mid On / Off Voltage of the TP5100 Charge LED Pins
...
void fanController()
{
	...
	{
		if (module[j].cycleState == 5 || module[j].cycleState == 2 || module[j].cycleState == 6)
		{
			fullSpeedFanOn = true;
		}
	}
	if (fullSpeedFanOn == true)
	{
		digitalWrite(FAN, HIGH); // Fan full speed if any Modules are running a Charge, Recharge or Discharge Cycle
	}
	else
	{
	...
	while (ESP8266.available() > 0)
	{
		...
		if (recievedMessage.length() > 1) // Multiple CHAR
		{
			for (byte i = 0; i <= recievedMessage.length(); i++)
			{
				...
				Serial.print(i);
				Serial.print(F(" Char: "));
				Serial.println(recievedMessage.charAt(i));
				if (recievedMessage.charAt(i) == ':' || recievedMessage.length() == i) // ???  recievedMessage.length() == (i - 1) because it starts counting at 0 ???
				// if (recievedMessage.charAt(i) == ':' || recievedMessage.length() == (i - 1))
				{
...
void returnCodes(int codeID)
{
	...
	default:
		...
		{
			moduleID = codeID - 100;
			module[moduleID].batteryBarcode = true;
		...
		{
			moduleID = codeID - 200;
			module[moduleID].insertData = true;
