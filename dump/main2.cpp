...
RotaryEncoder encoder(CLK, DT); // Define the decoder encoder Pin A (CLK) = 3 encoder Pin B (DT) = 4 | Pin 7 (CLK) and Pin 6 (DT) for Version 2.0
...
bool dischargeCycle(byte j)
{
  ...
  if (module[j].intMilliSecondsCount >= settings.dischargeReadInterval || module[j].dischargeAmps == 0) // Get reading every 5+ seconds or if dischargeAmps = 0 then it is first run
  {
    ...
    float batteryShuntVoltage = readVoltage(module[j].batteryVolatgeDropPin);
...
*/
  ...
  int rotaryEncoderDirection = encoder.getPosition(); // Check for rotation
